...
RAW_DATA <- RAW_DAT %>% left_join(., Periode_SST, by = "NIDO") %>% glimpse()
```
...
Tab <- RAW_DATA %>% 
  ...
  mutate(gr_Fage = cut_number(AGEHEMB, n = 5)) %>% 
  mutate(gr_Mage = cut_number(AGEMACH, n = 5)) %>% 
...
ggarrange(`Plot_2_Female ages`, 
  ...
  ggexport(filename = "ReproductiveValueArrange_CI2.png", width = 1500, height = 1700)
